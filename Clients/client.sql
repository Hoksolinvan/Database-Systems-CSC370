...
#Table Creation
create table `Clients`(`Client_ID` int primary key, `AccountNumber` int, `ClientName` varchar(40), `CheckingBalance` varchar(40) default '$0', `SavingBalance` varchar(40) default '$0', `AccountType` enum('None','Unleaded','Premium'), `CreditScore` int default 0);
...
select * from `Clients` order by `SavingBalance` desc LIMIT 1;


#Query for Customer1. They only want to see their Account Number, checking balance and saving balance
select `AccountNumber`,`CheckingBalance`,`SavingBalance` from `Clients` where `ClientName`='Customer 1';
